...
Feature: Content variant localization
  ...
  Scenario: Enable configuration "Automatically Switch Localization Based on URL"
    Given I follow "System Configuration/General Setup/Localization" on configuration sidebar
    And uncheck "Use default" for "Automatically Switch Localization Based on URL" field
    And fill form with:
      | Automatically Switch Localization Based on URL | true |
    When I submit form
    Then I should see "Configuration saved" flash message
  ...
  Scenario: Switch localization and check landing page content localization
    ...
    And select "German Localization" localization
    And click "Product Variant"
