...
/**
 * Loads data for expanded entities (requested by the "include" filter) for ProductSearch entity.
 */
class LoadProductSearchExpandedEntities implements ProcessorInterface
{
    ...
    private function getExpandedAssociationNames(EntityDefinitionConfig $config): array
    {
        ...
        foreach ($fields as $fieldName => $field) {
            ...
            if (!$this->isEntityExpanded($targetConfig)) {
                continue;
            }
     ...
     * @return bool
     */
    private function isEntityExpanded(EntityDefinitionConfig $config): bool
    {
        $result = false;
        ...
        if ($idFieldName) {
            $fields = $config->getFields();
            foreach ($fields as $fieldName => $field) {
                if (!$field->isExcluded() && $fieldName !== $idFieldName) {
                    $result = true;
                    break;
                }
            }
