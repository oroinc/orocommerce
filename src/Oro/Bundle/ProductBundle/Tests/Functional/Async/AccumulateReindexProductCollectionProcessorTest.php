...
use Oro\Bundle\ProductBundle\Async\AccumulateReindexProductCollectionProcessor;
 ...
 * @dbIsolationPerTest
 */
class AccumulateReindexProductCollectionProcessorTest extends WebTestCase
{
    use JobsAwareTestTrait;

    private AccumulateReindexProductCollectionProcessor $processor;
    ...
    protected function setUp(): void
    {
        ...
        $this->processor = self::getContainer()->get('oro_product.async.accumulate_product_collection_processor');
    }
    ...
    public function testProcess(bool $isFull, array $expectedProductRefs, array $additionalProductRefs): void
    {
        ...
        $messageData = self::getContainer()
            ->get('oro_product.model.accumulate_segment_message_factory')
            ->createMessage(
                $childJob->getId(),
                [$website->getId()],
                $segment,
