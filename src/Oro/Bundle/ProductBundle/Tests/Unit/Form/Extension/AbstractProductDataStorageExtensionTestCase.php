...
abstract class AbstractProductDataStorageExtensionTestCase extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->dataClass = get_class($this->getTargetEntity());
        ...
        $this->doctrineHelper->expects($this->any())
            ...
            ->willReturnCallback(function (string $entityClass, mixed $entityId) {
                return $this->entityManager->getReference($entityClass, $entityId);
            });
        $this->doctrineHelper->expects($this->any())
            ->method('getSingleEntityIdentifier')
            ->willReturnCallback(function (object $entity) {
                $id = $this->entityManager->getClassMetadata(get_class($entity))->getIdentifierValues($entity);

                return count($id) > 0 ? reset($id) : null;
            });
    }
