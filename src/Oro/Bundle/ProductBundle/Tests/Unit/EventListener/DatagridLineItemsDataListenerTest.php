...
class DatagridLineItemsDataListenerTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $localizationHelper
            ...
            ->willReturnCallback(
                static function (Collection $values) {
                    return $values->first();
                }
            );
        ...
        $attachmentManager
            ...
            ->willReturnCallback(
                static function (File $file, string $filterName) {
                    return $file->getFilename() . '_' . $filterName;
                }
            );
    ...
    public function onLineItemDataDataProvider(): array
    {
        ...
        $product1 = (new ProductStub())
            ->setId(1)
            ->setSku('p1')
            ->setNames([(new LocalizedFallbackValue())->setString($product1Name = 'Product1')]);
        ...
        $parentProduct = (new ProductStub())
            ->setId(2)
            ->setNames([(new LocalizedFallbackValue())->setString($parentProductName = 'Product2')]);
        ...
        $productWithImage = (new ProductStub())
            ->setId(3)
            ->setSku('p3')
            ->setNames([(new LocalizedFallbackValue())->setString($product3Name = 'Product3')]);
        ...
        $productWithUnitPrecision = (new ProductStub())
            ->setId(4)
            ->setSku('p4')
            ->setNames([(new LocalizedFallbackValue())->setString($product4Name = 'Product4')])
        ...
        $productWithDisabledUnitPrecision = (new ProductStub())
            ->setId(5)
            ->setSku('p5')
            ->setNames([(new LocalizedFallbackValue())->setString($product5Name = 'Product5')])
