...
class FrontendProductDatagridListenerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testOnResultAfterWithoutUrlHelper(): void
    {
        $imageUrl = '/image/1/medium';
        $record = new ResultRecord(
            [
                'id' => 1,
                'image_product_medium' => $imageUrl,
            ]
        );
        ...
        $this->listener->setUrlHelper(null);
        $this->listener->onResultAfter($event);

        self::assertTrue($record->getValue('hasImage'));
        self::assertEquals($imageUrl, $record->getValue('image'));
    }
}
