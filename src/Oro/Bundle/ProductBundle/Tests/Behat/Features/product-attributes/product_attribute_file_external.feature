...
Feature: Product attribute file external
  In order to have custom attributes for Product entity
  As an Administrator
  I need to be able to add File product attribute with Stored Externally
  ...
  Scenario: Check the error when no external files and images are allowed
    ...
    And I fill "Product Form" with:
      | FileField | http://example.org/image.png |
    And I save and close form
    Then I should see "Product Form" validation errors:
      | FileField | No external files and images are allowed. Allowed URLs RegExp can be configured on the following page: System -> Configuration -> General Setup -> Upload Settings. |
  ...
  Scenario: Check the error when the URL does not match allowed
    ...
    And I save and close form
    Then I should see "Product Form" validation errors:
      | FileField | The provided URL does not match the URLs allowed in the system configuration. |
  ...
  Scenario: Check the error when invalid MIME type
    ...
    When I fill "FileField" with absolute URL "/media/cache/fixtures/image.jpg" in form "Product Form"
    And I save and close form
    Then I should see "Product Form" validation errors:
      | FileField | The MIME type of the file is invalid ("image/jpeg"). Allowed MIME types are "image/png". |

  Scenario: Specify the correct external file URL to FileField
    When I fill "FileField" with absolute URL "/media/cache/fixtures/image.png" in form "Product Form"
    And I save and close form
