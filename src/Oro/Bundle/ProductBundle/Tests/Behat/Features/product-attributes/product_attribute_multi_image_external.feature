...
Feature: Product attribute multi image external
  In order to have custom attributes for Product entity
  As an Administrator
  I need to be able to add Multiple Images product attribute with Stored Externally
  ...
  Scenario: Check the error when no external files and images are allowed
    ...
    And I fill "Product Form" with:
      | MultiImageField 1 | http://example.org/image.png |
    And I save and close form
    Then I should see "Product Form" validation errors:
      | MultiImageField 1 | No external files and images are allowed. Allowed URLs RegExp can be configured on the following page: System -> Configuration -> General Setup -> Upload Settings. |
  ...
  Scenario: Check the error when the URL does not match allowed
    ...
    And I save and close form
    Then I should see "Product Form" validation errors:
      | MultiImageField 1 | The provided URL does not match the URLs allowed in the system configuration. |
  ...
  Scenario: Check the error when invalid MIME type
    ...
    When I fill "MultiImageField 1" with absolute URL "/media/cache/fixtures/image.jpg" in form "Product Form"
    And I save and close form
    Then I should see "Product Form" validation errors:
      | MultiImageField 1 | The MIME type of the file is invalid ("image/jpeg"). Allowed MIME types are "image/png". |

  Scenario: Specify the correct external file URL to MultiImageField 1
    When I fill "MultiImageField 1" with absolute URL "/media/cache/fixtures/image.png" in form "Product Form"
    And I save and close form
  ...
  Scenario: Check file attribute is available at store front
    ...
    Then I should see "MultiImageField"
    And image "MultiImageField 1 Image" is loaded
    When I click "MultiImageField 1 Gallery Trigger"
    Then I should see images in "Popup Gallery Widget" element
    And image "Popup Gallery Widget Image" is loaded

  Scenario: Delete product attribute
    Given I proceed as the Admin
    And I go to Products/ Product Attributes
    When I click Remove "MultiImageField" in grid
