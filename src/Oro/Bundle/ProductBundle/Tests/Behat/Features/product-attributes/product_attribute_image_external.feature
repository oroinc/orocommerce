...
Feature: Product attribute image external
  In order to have custom attributes for Product entity
  As an Administrator
  I need to be able to add Image product attribute with Stored Externally
  ...
  Scenario: Check the error when no external files and images are allowed
    ...
    And I fill "Product Form" with:
      | ImageField | http://example.org/image.png |
    And I save and close form
    Then I should see "Product Form" validation errors:
      | ImageField | No external files and images are allowed. Allowed URLs RegExp can be configured on the following page: System -> Configuration -> General Setup -> Upload Settings. |
  ...
  Scenario: Check the error when the URL does not match allowed
    ...
    And I save and close form
    Then I should see "Product Form" validation errors:
      | ImageField | The provided URL does not match the URLs allowed in the system configuration. |
  ...
  Scenario: Check the error when invalid MIME type
    ...
    When I fill "ImageField" with absolute URL "/media/cache/fixtures/image.jpg" in form "Product Form"
    And I save and close form
    Then I should see "Product Form" validation errors:
      | ImageField | The MIME type of the file is invalid ("image/jpeg"). Allowed MIME types are "image/png". |

  Scenario: Specify the correct external file URL to ImageField
    When I fill "ImageField" with absolute URL "/media/cache/fixtures/image.png" in form "Product Form"
    And I save and close form
  ...
  Scenario: Check file attribute is available at store front
    ...
    And image "ImageField Image" is loaded
    When I click "ImageField Gallery Trigger"
