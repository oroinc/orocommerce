...
use Oro\Bundle\CustomerBundle\EventListener\CategoryTreeHandlerListener;
use Oro\Bundle\CustomerBundle\Provider\AccountUserRelationsProvider;
use Oro\Bundle\CustomerBundle\Visibility\Resolver\CategoryVisibilityResolverInterface;
...
class CategoryTreeHandlerListenerTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        $this->categoryVisibilityResolver = $this
            ->getMockBuilder('Oro\Bundle\CustomerBundle\Visibility\Resolver\CategoryVisibilityResolverInterface')
            ->disableOriginalConstructor()
            ->getMock();

        $this->accountUserRelationsProvider = $this
            ->getMockBuilder('Oro\Bundle\CustomerBundle\Provider\AccountUserRelationsProvider')
            ->disableOriginalConstructor()
            ->getMock();
    ...
    public function onCreateAfterDataProvider()
    {
        return [
            ...
            'tree for account user with invisible ids' => [
                ...
                'account' => $this->getEntity('Oro\Bundle\CustomerBundle\Entity\Account', ['id' => 42])
            ]
        ];
    }
