...
Feature: Create Order from RFQ with Product Kits - with Removed Kit Item Product
  ...
  Scenario: Create Order from the RFQ and check the Create Order form
    ...
    And I should see "The currency should not be blank." in the "Order Line Item Draft Edit Form Errors" element
    And I should see "No matching price found" in the "Order Line Item Draft Edit Form Errors" element
    And I should see "Order Line Item Draft Edit Form" validation errors:
      | ProductKitItem2Product | The selected product is not allowed No matching price found. |
    And I should see "No matching price found" in the "Order Line Item Draft Edit Form Kit Item 1 Errors" element
  ...
  Scenario: Replace the missing kit item product and save the Order successfully
    ...
    Then I should see following "Order Line Item Draft Grid" grid:
      ...
      | product-kit-01    | Product Kit 01 Optional Item [piece x 2] $3.70 Simple Product 03 Mandatory Item [piece x 1] $2.47 Simple Product 02 | 1 piece  | $133.34 |
    When I save form
