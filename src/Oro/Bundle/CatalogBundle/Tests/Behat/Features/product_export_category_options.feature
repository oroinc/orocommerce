...
Feature: Product Export Category Options
  ...
  Scenario: Export products with category default title by default
    ...
    And exported file contains at least the following columns:
      | SKU   | Category.ID | category.default.title |
      | PSKU1 | 3           | Laptops                |
      | PSKU2 | 4           | Phones                 |
      | PSKU3 | 2           | Electronics            |
  ...
  Scenario: Enable category path export
    ...
    And I check "Export Category Path"
    When I save form
  ...
  Scenario: Disable category default title export
    Given I go to System/ Configuration
    And I follow "Commerce/Product/Product Import\/Export" on configuration sidebar
    And uncheck "Use default" for "Export Category Default Title" field
    And I uncheck "Export Category Default Title"
    When I save form
    Then I should see "Configuration saved" flash message
