<?php

namespace Oro\Bundle\WebsiteSearchBundle\Entity;

use Doctrine\ORM\Mapping as ORM;
use Oro\Bundle\SearchBundle\Entity\AbstractIndexDecimal;

/**
 * Stores values of decimal fields
 */
#[ORM\Entity]
#[ORM\Table(name: 'oro_website_search_decimal')]
#[ORM\Index(columns: ['field'], name: 'oro_website_search_decimal_field_idx')]
#[ORM\Index(columns: ['item_id', 'field'], name: 'oro_website_search_decimal_item_field_idx')]
class IndexDecimal extends AbstractIndexDecimal
{
    #[ORM\ManyToOne(targetEntity: Item::class, inversedBy: 'decimalFields')]
    #[ORM\JoinColumn(name: 'item_id', referencedColumnName: 'id', nullable: false, onDelete: 'CASCADE')]
    protected ?Item $item = null;
}
