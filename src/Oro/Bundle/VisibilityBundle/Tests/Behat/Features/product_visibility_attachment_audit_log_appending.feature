...
Feature: Product Visibility Attachment Audit Log Appending
  ...
  Scenario: Should see audit log once add attachment for current product
    ...
    Then should see following "Audit History Grid" grid:
      ...
      | Attachment: | Attachment:  Attachment "Item #1" added: File: File "57" Product: Product "1" |
  ...
  Scenario: Delete attachment
    ...
    Then should see following "Audit History Grid" grid:
      | Old Values                                                                      | New values  |
      | Attachment: Attachment "Item #1" removed: File: File "57" Product: Product "1" | Attachment: |
    And I close ui dialog
  ...
  Scenario: Should see audit log once update visibility to customer for current product
    ...
    Then should see following "Audit History Grid" grid:
      | Old Values                                                                                                       | New values                                                                                                               |
      | Product Visibility to Customers: Product Visibility to Customers "current_product" changed: Visibility: category | Product Visibility to Customers:  Product Visibility to Customers "current_product" changed: Visibility: current_product |
    And I close ui dialog
