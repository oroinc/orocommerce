...
class ShoppingListLineItemDiffMapper implements CheckoutStateDiffMapperInterface
{
    ...
    private function getCompareString(ShippingLineItemInterface $item): string
    {
        ...
        $weightUnit = $weight ? $weight->getUnit() : null;
        $dimension = $item->getDimensions();
        $dimensionValue = $dimension? $dimension->getValue() : null;
        $dimensionUnit = $dimension ? $dimension->getUnit() : null;
        $product = $item->getProduct();

        return sprintf(
            "s%s-u%s-q%d-p%s%d-w%d%s-d%dx%dx%d%s-i%s",
            $product ? $product->getSkuUppercase() : "",
            $item->getProductUnitCode(),
            $item->getQuantity(),
            $price ? $price->getCurrency() : "",
            $price ? $price->getValue() : "",
            $weight ? $weight->getValue() : "",
            $weightUnit ? $weightUnit->getCode() : "",
            $dimensionValue ? $dimensionValue->getLength() : "",
            $dimensionValue ? $dimensionValue->getLength() : "",
            $dimensionValue ? $dimensionValue->getLength() : "",
            $dimensionUnit ? $dimensionUnit->getCode() : "",
            $product ? $product->getInventoryStatus()->getId() : ""
        );
    }
     ...
     * @return mixed|null
     */
    private function getCheckoutConfig($entity): ?bool
    {
