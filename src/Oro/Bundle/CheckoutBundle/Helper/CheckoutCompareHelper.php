...
class CheckoutCompareHelper
{
    ...
    private function startShoppingListCheckoutAction(ShoppingList $shoppingList, $forceStartCheckout = false): void
    {
        ...
        $actionGroup = $this->actionGroupRegistry->findByName('start_shoppinglist_checkout');
    ...
    public function getNewCheckoutItemChangedConfig(array $sourceCriteria): bool
    {
        $shoppingList = $sourceCriteria['shoppingList'] ?? null;
        $originalScopeId = $this->configManager->getScopeId();
        if ($shoppingList instanceof ShoppingList) {
            $organization = $shoppingList->getOrganization();
