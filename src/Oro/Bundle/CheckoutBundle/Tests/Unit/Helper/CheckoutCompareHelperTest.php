...
{
    /** @var CheckoutDiffStorageInterface|MockObject */
    private $diffStorage;

    /** @var ShoppingListLineItemDiffMapper|MockObject */
    private $diffMapper;
    ...
    public function testGetNewCheckoutItemChangedConfig(): void
    {
        ...
        $this->assertTrue($this->helper->getNewCheckoutItemChangedConfig(['shoppingList' => $shoppingList]));
    }
    ...
    /**
     * @param MockObject|Checkout $checkout
     * @param string $token
     * @return WorkflowItem|MockObject
     */
    private function getWorkflowItem($checkout, string $token)
    {
        $workflowItem = $this->createMock(WorkflowItem::class);
        $workflowData = $this->createMock(WorkflowData::class);
        $this->workflowManager->expects($this->once())
