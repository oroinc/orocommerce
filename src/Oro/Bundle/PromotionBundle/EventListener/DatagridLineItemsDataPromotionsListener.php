...
class DatagridLineItemsDataPromotionsListener
{
    ...
    private ?RoundingServiceInterface $roundingService = null;
    ...
    public function onLineItemData(DatagridLineItemsDataEvent $event): void
    {
        ...
        foreach ($lineItems as $lineItem) {
            ...
            $subtotalValue = BigDecimal::of(($lineItemData[DatagridLineItemsDataPricingListener::SUBTOTAL_VALUE] ?? 0))
                ->minus($discountValue)
                ->toFloat();
            $subtotalValue = $this->round($subtotalValue);
    ...
    private function round(float $subtotalValue): int|float
    {
        return $this->roundingService !== null ? $this->roundingService->round($subtotalValue) : $subtotalValue;
    }
}
