...
final class OrderDuplicatorTest extends DuplicatorTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->duplicator = new OrderDuplicator($this->duplicatorFactory);
        $this->duplicator->setEventDispatcher($this->eventDispatcher);
    }

    public function testDuplicateWithoutEventDispatcherDoesNotDispatchEvent(): void
    {
        $duplicator = new OrderDuplicator($this->duplicatorFactory);
        // No event dispatcher set - eventDispatcher remains null.

        $order = new Order();
        ReflectionUtil::setId($order, 4100);

        $duplicatedOrder = $duplicator->duplicate($order);
