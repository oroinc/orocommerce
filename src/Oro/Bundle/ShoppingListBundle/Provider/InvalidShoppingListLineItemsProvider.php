...
class InvalidShoppingListLineItemsProvider implements ResetInterface
{
    ...
    public function getInvalidItemsViolations(Collection|array $lineItems, ?string $validationGroupType = null): array
    {
        ...
        // Preload line items data before validation to avoid N+1 queries
        $this->preloadLineItemsData($lineItems);
    ...
    private function getCacheKey(Collection|array $lineItems, array $validationGroups): string
    {
        ...
        foreach ($lineItems as $lineItem) {
            if ($lineItem instanceof ProductLineItemInterface) {
                $id = $lineItem->getEntityIdentifier();
                if ($id !== null) {
                    $ids[] = $id;
                }
            }
    ...
    /**
     * Preloads line items data to avoid N+1 queries during validation.
     * Loads Doctrine associations like unitPrecisions, product, etc.
     */
    private function preloadLineItemsData(Collection|array $lineItems): void
    {
        if (!$lineItems) {
            return;
        }
        ...
        $lineItemsArray = $lineItems instanceof Collection ? $lineItems->toArray() : $lineItems;

        if (empty($lineItemsArray)) {
            return;
        }

        // Preload fields needed for validation
        // This loads unitPrecisions, fallback values and other Doctrine associations
        // needed for HasEnoughInventoryLevelValidator and QuantityToOrderValidator
        $this->preloadingManager->preloadInEntities($lineItemsArray, [
            'product' => [
                'unitPrecisions' => [],
                'names' => [],
                // Fallback values used by InventoryQuantityManager (HasEnoughInventoryLevelValidator)
                'manageInventory' => [],
                'decrementQuantity' => [],
                'backOrder' => [],
                'inventoryThreshold' => [],
                // Fallback values used by other validators
                'highlightLowInventory' => [],
                'isUpcoming' => [],
                'maximumQuantityToOrder' => [],
                'minimumQuantityToOrder' => [],
                // Fallback values from category (used by QuantityToOrderValidator and fallback resolution)
                'category' => [
                    'highlightLowInventory' => [],
                    'isUpcoming' => [],
                    'maximumQuantityToOrder' => [],
