...
/**
 * Defines default options for remove from saved for later mass action.
 */
class RemoveFromSavedForLaterMassAction extends AjaxMassAction
{
    #[\Override]
    public function setOptions(ActionConfiguration $options): ActionInterface
    {
        if (!isset($options['handler'])) {
            $options['handler'] = 'oro_shopping_list.mass_action.remove_from_saved_for_later_handler';
        }

        if (!isset($options['frontend_type'])) {
            $options['frontend_type'] = 'remove-from-saved-for-later-mass';
        }
