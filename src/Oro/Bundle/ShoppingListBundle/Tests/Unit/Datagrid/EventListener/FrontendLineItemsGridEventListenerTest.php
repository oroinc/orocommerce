        ...
        $urlGenerator
            ...
            ->willReturnCallback(
                static function (string $name, array $parameters) {
                    return $name . '_' . implode('_', array_merge(array_keys($parameters), array_values($parameters)));
                }
            );
    ...
    private function getProduct(int $id): Product
    {
        return $this->getEntity(
            ...
            [
                'id' => $id,
                'sku' => 'sku' . $id,
                'names' => new ArrayCollection([(new LocalizedFallbackValue())->setString('name' . $id)]),
            ]
        );
    }
