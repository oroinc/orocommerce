...
class ApplicationEntitiesOrmIndexerTest extends AbstractEntitiesOrmIndexerTest
{
    ...
    protected function setUp(): void
    {
        ...
        $productInventoryStatus = $this->getReference('in_stock');
        $product = (new Product())
            ->setOrganization($organization)
            ->setOwner($businessUnit)
            ->setSku('TEST-SKU')
        ...
        $orderInternalStatus = $this->getReference(
            'order_internal_status.' . OrderStatusesProviderInterface::INTERNAL_STATUS_OPEN
        );
        $order = (new Order())
            ->setOrganization($organization)
            ->setOwner($user)
        ...
        $requestCustomerStatus = $this->getEnumEntity(Request::CUSTOMER_STATUS_CODE, 'submitted');
        $requestInternalStatus = $this->getEnumEntity(Request::INTERNAL_STATUS_CODE, 'open');
        $request = (new Request())
            ->setOrganization($organization)
            ->setOwner($user)
            ->setCustomerUser($customerUser)
        ...
        $quoteCustomerStatus = $this->getEnumEntity(Request::CUSTOMER_STATUS_CODE, 'open');
        $quoteInternalStatus = $this->getEnumEntity(Request::INTERNAL_STATUS_CODE, 'sent_to_customer');
        $quote = (new Quote())
            ->setOrganization($organization)
            ->setOwner($user)
    ...
    private function getEnumEntity(string $enumField, string $enumCode): AbstractEnumValue
    {
        ...
        return $entityManager->getReference($className, $enumCode);
    }
}
