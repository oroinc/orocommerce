...
class LocalizedSlugType extends AbstractType
{
    ...
    public function getSlugifyEntityHelper(): SlugifyEntityHelper
    {
        if (!$this->slugifyEntityHelper) {
            throw new \LogicException('SlugifyEntityHelper can not be null.');
        }

        return $this->slugifyEntityHelper;
    }
    ...
    public function onPostSubmit(FormEvent $event): void
    {
        ...
        if ($form->has($sourceFieldName)) {
            ...
            if ($localizedSources instanceof Collection && $localizedSlugs instanceof Collection) {
                $this->getSlugifyEntityHelper()->fillFromSourceField($localizedSources, $localizedSlugs);
            }
        }
    }
