...
class ProductLineItemProductPriceProviderTest extends TestCase
{
    ...
    /**
     * @dataProvider notSupportedKitItemLineItemDataProvider
     */
    public function testGetProductLineItemProductPricesWhenIsProductKitAndKitItemLineItemNotSupported(
        ProductKitItemLineItemsAwareStub $lineItem
    ): void {
        $this->productPriceCriteriaFactory
            ->expects(self::never())
            ->method(self::anything());
    ...
    public function notSupportedKitItemLineItemDataProvider(): array
    {
        $unitItem = (new ProductUnit())->setCode('item');
        $kitItemProduct = (new ProductStub())->setId(100);
        $createKitItem = static fn () => (new ProductKitItemStub(10))
            ->setProductUnit($unitItem)
            ->addKitItemProduct((new ProductKitItemProduct())->setProduct($kitItemProduct));
        $createProduct = static fn (ProductKitItemStub $kitItem) => (new ProductStub())
            ->setId(42)
            ->addUnitPrecision((new ProductUnitPrecision())->setUnit($unitItem))
            ->setType(Product::TYPE_KIT)
            ->addKitItem($kitItem);
        $createLineItem = static fn (
            ProductStub $product,
            ProductKitItemLineItemStub $kitItemLineItem
        ) => (new ProductKitItemLineItemsAwareStub(42))
            ->setProduct($product)
            ->setUnit($unitItem)
            ->setQuantity(23.4567)
            ->addKitItemLineItem($kitItemLineItem);

        $kitItemWithoutUnit = $createKitItem();
        $kitItemLineItemWithoutUnit = (new ProductKitItemLineItemStub(1000))
            ->setKitItem($kitItemWithoutUnit)
            ->setProduct($kitItemProduct)
            ->setQuantity(1.2345);

        $kitItemWithoutQuantity = $createKitItem();
        $kitItemLineItemWithoutQuantity = (new ProductKitItemLineItemStub(1000))
            ->setKitItem($kitItemWithoutQuantity)
            ->setProduct($kitItemProduct)
            ->setUnit($unitItem)
            ->setQuantity(null);

        $kitItemWithZeroQuantity = $createKitItem();
        $kitItemLineItemWithZeroQuantity = (new ProductKitItemLineItemStub(1000))
            ->setKitItem($kitItemWithZeroQuantity)
            ->setProduct($kitItemProduct)
            ->setUnit($unitItem)
            ->setQuantity(0.0);

        return [
            'kit item line item without product unit' => [
                'lineItem' => $createLineItem(
                    $createProduct($kitItemWithoutUnit),
                    $kitItemLineItemWithoutUnit
                ),
            ],
            'kit item line item without quantity' => [
                'lineItem' => $createLineItem(
                    $createProduct($kitItemWithoutQuantity),
                    $kitItemLineItemWithoutQuantity
                ),
            ],
            'kit item line item with non-positive quantity' => [
                'lineItem' => $createLineItem(
                    $createProduct($kitItemWithZeroQuantity),
                    $kitItemLineItemWithZeroQuantity
                ),
            ],
        ];
    }
