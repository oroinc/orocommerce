...
abstract class AbstractPricesCombiningStrategyTest extends WebTestCase
{
    ...
    protected function assertMqMessagesSent(
        int $expectedIndexMessagesCount,
        array $expectedProductIds
    ): void {
        $collector = self::getMessageCollector();
        $messages = $collector->getTopicSentMessages(AsyncIndexer::TOPIC_REINDEX);
        $this->assertCount($expectedIndexMessagesCount, $messages, 'Unexpected MQ messages count');
        ...
        $this->assertEquals($expectedProductIds, $messageProductIds, 'Unexpected products sent for indexation');
    }
