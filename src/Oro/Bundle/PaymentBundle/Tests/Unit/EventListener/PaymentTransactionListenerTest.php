...
class PaymentTransactionListenerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->listener->setDoctrine($this->doctrine);
    }
    ...
    public function testOnTransactionCompleteWithNullDoctrine(): void
    {
        $paymentTransaction = new PaymentTransaction();
        $paymentTransaction->setEntityClass(Order::class);
        $paymentTransaction->setEntityIdentifier(123);
        ...
        // Create listener without doctrine for BC layer
        $listener = new PaymentTransactionListener($this->manager);

        $event = new TransactionCompleteEvent($paymentTransaction);
        $listener->onTransactionComplete($event);
    }

    public function testOnTransactionCompleteWithNullDoctrineAndDifferentEntityType(): void
    {
        $paymentTransaction = new PaymentTransaction();
        $paymentTransaction->setEntityClass(\stdClass::class);
        $paymentTransaction->setEntityIdentifier(789);
