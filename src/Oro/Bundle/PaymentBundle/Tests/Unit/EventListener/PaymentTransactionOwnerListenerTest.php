...
class PaymentTransactionOwnerListenerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testPrePersistOnNonSupportedEntity()
    {
        $this->tokenAccessor->expects($this->never())
            ->method('hasUser');

        $eventArgs = new LifecycleEventArgs(new \stdClass(), $this->createMock(ObjectManager::class));
    ...
    public function testPrePersistOnEmptyToken()
    {
        ...
        $eventArgs = new LifecycleEventArgs($transaction, $this->createMock(ObjectManager::class));
        $this->listener->prePersist($eventArgs);
