...
namespace Oro\Bundle\PaymentBundle\Datagrid\Extension\MassAction;
...
class StatusMassActionHandler implements MassActionHandlerInterface
{
     ...
     * @var string
     */
    protected $responseMessage = 'oro.payment.datagrid.status.success_message';
    ...
    public function handle(MassActionHandlerArgs $args)
    {
        ...
        try {
            set_time_limit(0);
            $iteration = $this->handlePaymentMethodsConfigsRuleStatuses($options, $data);
     ...
     * @return int
     */
    protected function handlePaymentMethodsConfigsRuleStatuses($options, $data)
    {
        $status = $options['enable'];
        $isAllSelected = $this->isAllSelected($data);
        $iteration = 0;
        $paymentMethodsConfigsRuleIds = [];

        if (array_key_exists('values', $data) && !empty($data['values'])) {
            $paymentMethodsConfigsRuleIds = explode(',', $data['values']);
        }

        if ($paymentMethodsConfigsRuleIds || $isAllSelected) {
            $queryBuilder = $this
                ->entityManager
    ...
    protected function process($queryBuilder, $status, $iteration)
    {
        ...
        foreach ($result as $entity) {
            /** @var PaymentMethodsConfigsRule $entity */
            $entity = $entity[0];
