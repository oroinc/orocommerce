...
class FormViewListenerTest extends FormViewListenerTestCase
{
    ...
    public function testOnOrderViewWithoutRequest()
    {
        $this->requestStack->expects($this->once())->method('getCurrentRequest')->willReturn(null);
        ...
        $this->listener->onOrderView($event);
    }

    public function testOnOrderViewWithoutOrder()
    {
        $this->requestStack->expects($this->once())->method('getCurrentRequest')->willReturn($this->request);
        $this->request->expects($this->once())->method('get')->with('id')->willReturn(8);

        $this->doctrineHelper->expects($this->once())
            ->method('getEntityReference')
            ->with('OroOrderBundle:Order', 8)
            ->willReturn(null);
    ...
    public function testOnOrderView()
    {
        $this->requestStack->expects($this->once())
            ->method('getCurrentRequest')
            ->willReturn($this->request);
