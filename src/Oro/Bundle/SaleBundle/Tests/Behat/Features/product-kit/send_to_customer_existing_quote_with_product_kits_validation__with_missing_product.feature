...
@fixture-OroSaleBundle:product-kit/send_to_customer_existing_quote_with_product_kits_validation__with_missing_product__quote.yml

Feature: Send to Customer existing Quote with Product Kits Validation - with Missing Product
  ...
  Scenario: Check the kit item line items with a missing and disabled products on the Quote view page
    ...
    Scenario: Try to send the Quote to customer
      Given "Send to Customer" button is not disabled
      When I click "Send to Customer"
      And click "Send"
      Then I should see "You have been redirected to the Quote Edit page as a Quote has not acceptable configuration and requires your attention. Please, review and update it to proceed." flash message
  ...
  Scenario: Send the Quote to customer
    Given "Send to Customer" button is not disabled
    When I click "Send to Customer"
    And click "Send"
    Then I should see "Quote #Quote1 successfully sent to customer" flash message
    And I should see Quote with:
